...
class FuzzyPlayer(Player):
    def __init__(self, racket: Racket, ball: Ball, board: Board):
        ...
        x_dist = fuzz.control.Antecedent(np.arange(40, 200, 0.5), 'x')
        y_dist = fuzz.control.Antecedent(np.arange(0, 400, 0.5), 'y')
        speed = fuzz.control.Consequent(np.arange(5, 50, 0.001), 'speed')
        names = ['nb', 'ns', 'ze', 'ps', 'pb']
        x_dist.automf(names=names)
        y_dist.automf(names=names)
        speed.automf(names=names)
        x_dist.view()
        y_dist.view()
        speed.view()
        rule0 = fuzzcontrol.Rule(antecedent=((x_dist['nb'] & y_dist['pb']) |
                                             (x_dist['nb'] & y_dist['ps'])),
                          consequent=speed['nb'], label='rule nb')

        rule1 = fuzzcontrol.Rule(antecedent=((x_dist['ns'] & y_dist['ze'])|
                                             (x_dist['nb'] & y_dist['ze'])|
                                             (x_dist['ze'] & y_dist['nb'])),
                          consequent=speed['ns'], label='rule ns')

        rule2 = fuzzcontrol.Rule(antecedent=((x_dist['ze'] & y_dist['pb']) |
                                              (x_dist['ze'] & y_dist['ps'])),
                          consequent=speed['ze'], label='rule ze')

        rule3 = fuzzcontrol.Rule(antecedent=((x_dist['ze'] & y_dist['ze'])|
                                             (x_dist['nb'] & y_dist['nb'])|
                                             (x_dist['ns'] & y_dist['ns']) |
                                             (x_dist['ps'] & y_dist['pb'])|
                                             (x_dist['pb'] & y_dist['pb'])
        ...
        rule4 = fuzzcontrol.Rule(antecedent=((x_dist['ps'] & y_dist['ps'])|(x_dist['pb'] & y_dist['ze']) |
                                             (x_dist['pb'] & y_dist['nb'])
                                             ),
                          consequent=speed['pb'], label='rule pb')
        self.racket_controller = fuzz.control.ControlSystem(rules=[rule0, rule1, rule2, rule3,rule4])
        self.racket_controller = fuzz.control.ControlSystemSimulation(self.racket_controller)
    ...
    def make_decision(self, x_diff: int, y_diff: int):
        self.racket_controller.input['x'] = abs(x_diff)
        self.racket_controller.input['y'] = y_diff
        self.racket_controller.compute()
        if x_diff > 0:
            print(x_diff, y_diff, -self.racket_controller.output['speed'])
